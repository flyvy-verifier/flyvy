...
pub fn compute_lfp_single(
    ...
    let predicates = chc_sys
        ...
        .map(|decl| {
            ...
            if !decl.args.is_empty() {
                ...
                for imp_chc in &imp_chcs {
                    if imp_chc.relevant_for(&decl.name) {
                        ...
                        let (qf_bs, qf_ls) =
                            imp_chc.qf_leqs(&mining_tactic, &decl.args, &mut qf_int_terms);

                        for b in qf_bs {
                            if !qf_bool_terms.contains(&b) {
                                qf_bool_terms.push(b)
                            }
                        }

                        for (expr, r) in qf_ls {
                            qf_int_templates.add_range(expr, r);
                        }
                    }
                }
            ...
            let qf_context =
                PredicateConfig::qf_units(qf_int_terms, qf_bool_terms, qf_int_templates);

            let quant_context = PredicateConfig::quantified_disjunctions(
                int_terms,
                bool_terms,
                int_templates,
                quantified,
            ...
            PredicateConfig::from_quant_contexts(decl, vec![qf_context, quant_context])
        })
        .collect();
